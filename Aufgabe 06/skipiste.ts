...
namespace aufgabe6 {
    ...
    let object: MovingShape[] = [];
    let n: number = 3; //3 Skifahrer und 3 Wolken
    let f: number = 150; //150 Schneeflocken 
    ...
    function init(): void {
        ...
        //For Schleife für die Wolken und Skifahrer
        for (let i: number = 0; i < n; i++) {
            let c: InterWolke = new InterWolke(0 + Math.random() * 1000, 0 + Math.random() * 200, 2.5, 0, "white");
            object.push(c);
        ...
        for (let i: number = 0; i < f; i++) {
            let f: InterFlocke = new InterFlocke(0 + Math.random() * 1000, 0 + Math.random() * 600, 0, Math.random() + 10, "white");
            object.push(f);
        }
